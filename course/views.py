...
from .models import (
                    ...
                    Question,Answer,Reply,Discussion,Quiz
                    )
...
class TopicListCreateAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if qs.exists():
            syllabus_qs = Syllabus.objects.filter(syllabus_id=syllabus_id)
            if syllabus_qs.exists():
                topic_serialize = TopicSerializer(data=request.data)
                if topic_serialize.is_valid():
                    title = topic_serialize.validated_data.get("title")
                    topic_create = Topic.objects.create(**topic_serialize.validated_data)
                    topic_serialize.save(title=title)
                    syllabus_qs.first().topics.add(topic_create)
...
#         qs = Quiz.objects.filter(quiz_id=quiz_id)
#         if qs.exists():
#             serialize = QuizAnswerSerializer(data=request.data)
#             if serialize.is_valid():
